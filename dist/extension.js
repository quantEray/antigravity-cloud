...
</html>`;t.webview.html=S}};var z=A(require("vscode"));var H=class r{constructor(t){this._extensionUri=t}static viewType="antigravityAnywhereSidebarView";static currentView;_view;resolveWebviewView(t,o,e){this._view=t,r.currentView=t,t.webview.options={enableScripts:!0,localResourceRoots:[this._extensionUri]},t.webview.onDidReceiveMessage(async i=>{switch(i.command){case"googleLogin":await z.commands.executeCommand("antigravityAnywhere.googleLogin"),this.refresh();break;case"googleLogout":await z.commands.executeCommand("antigravityAnywhere.googleLogout"),this.refresh();break;case"deepScan":await z.commands.executeCommand("antigravityAnywhere.deepScan"),this.refresh();break;case"syncNow":await z.commands.executeCommand("antigravityAnywhere.syncNow"),this.refresh();break;case"restore":await z.commands.executeCommand("antigravityAnywhere.restore"),this.refresh();break;case"deleteAll":await z.commands.executeCommand("antigravityAnywhere.deleteAllFiles",!0),this.refresh();break;case"deleteFile":await z.commands.executeCommand("antigravityAnywhere.deleteFile",i.relativePath),this.refresh();break;case"deleteConversation":await z.commands.executeCommand("antigravityAnywhere.deleteConversation",i.convId),this.refresh();break;case"openDashboard":await z.commands.executeCommand("antigravityAnywhere.openDashboard");break;case"cancelSync":await z.commands.executeCommand("antigravityAnywhere.cancelSync"),this.refresh();break;case"toggleAutoSync":await z.commands.executeCommand("antigravityAnywhere.toggleAutoSync"),this.refresh();break;case"openGithubStar":z.env.openExternal(z.Uri.parse("https://github.com/quantEray/antigravity-cloud"));break;case"openGithubIssues":z.env.openExternal(z.Uri.parse("https://github.com/quantEray/antigravity-cloud/issues"));break;case"refresh":this.refresh();break}}),this.refresh()}async refresh(){this._view&&(this._view.webview.html=await this._getHtmlForWebview())}async _getHtmlForWebview(){let t=I(),o=Y.getDeviceInfo(),e=0,i="0",n="";try{let x=await P.scanDataDirectory(t.antigravityDataDir);i=x.totalSizeMB;let w=P.groupFilesByConversation(x,t.antigravityDataDir);e=w.length,n=w.map(v=>`<div class="conv-item">
              <div class="conv-header">
...
</html>`}};var K,F=null,Be="",G=null;function Ye(r){K=d.window.createStatusBarItem(d.StatusBarAlignment.Right,100),K.command="antigravityAnywhere.openDashboard",B("Not Logged In","$(account)"),K.show(),r.subscriptions.push(K);let t=new H(r.extensionUri);r.subscriptions.push(d.window.registerWebviewViewProvider(H.viewType,t)),N.validateOrRefreshToken().then(e=>{if(e){let i=I();B(i.googleUserName?`Signed in as ${i.googleUserName.split(" ")[0]}`:"Signed In","$(cloud-check)")}else B("Not Logged In","$(account)");t.refresh(),M.refreshCurrentPanel()}),r.subscriptions.push(d.commands.registerCommand("antigravityAnywhere.googleLogin",async()=>{try{B("Signing in...","$(sync~spin)"),await N.startLoginFlow();let e=I();B(e.googleUserName?`${e.googleUserName.split(" ")[0]}`:"Signed In","$(cloud-check)"),d.window.showInformationMessage("Antigravity Cloud: Successfully signed in with Google! You can now sync manually."),t.refresh(),M.refreshCurrentPanel()}catch(e){B("Login Error","$(error)"),d.window.showErrorMessage("Antigravity Cloud Google Login Failed: "+e.message)}}),d.commands.registerCommand("antigravityAnywhere.openDashboard",()=>M.show(r)),d.commands.registerCommand("antigravityAnywhere.cancelSync",()=>{G&&(G.abort(),G=null,B("Canceled","$(x)"),$(!1,0,"\u23F9\uFE0F Sync canceled by user.","Canceled"),d.window.showInformationMessage("Antigravity Cloud: Sync operation canceled by user."),t.refresh(),M.refreshCurrentPanel())}),d.commands.registerCommand("antigravityAnywhere.toggleAutoSync",async()=>{let e=I(),i=!e.enableAutoSync;await he(i),i?(e.googleDriveToken&&(F||(F=new V(e.syncIntervalSeconds,()=>re(!1))),F.startWatching(e.antigravityDataDir)),d.window.showInformationMessage("Antigravity Cloud: Automatic Sync is now ENABLED \u26A1")):(F&&(F.stopWatching(),F=null),d.window.showInformationMessage("Antigravity Cloud: Automatic Sync is now DISABLED \u{1F6D1} (Manual Sync only)")),t.refresh(),M.refreshCurrentPanel()}),d.commands.registerCommand("antigravityAnywhere.syncNow",async()=>{await re(!0),t.refresh(),M.refreshCurrentPanel()}),d.commands.registerCommand("antigravityAnywhere.restore",async()=>{await Ke(),t.refresh(),M.refreshCurrentPanel()}),d.commands.registerCommand("antigravityAnywhere.googleLogout",async()=>{await N.logout(),B("Not Logged In","$(account)"),d.window.showInformationMessage("Antigravity Cloud: Signed out of Google."),t.refresh(),M.refreshCurrentPanel()}),d.commands.registerCommand("antigravityAnywhere.deepScan",async()=>{await d.window.withProgress({location:d.ProgressLocation.Notification,title:"Antigravity Anywhere: Deep Scanning all conversation directories...",cancellable:!1},async()=>{let e=I(),i=await P.scanDataDirectory(e.antigravityDataDir),n=P.groupFilesByConversation(i);t.refresh(),d.window.showInformationMessage(`Antigravity Anywhere Deep Scan Complete: Found ${n.length} Conversations (${i.files.length} Total Files, ${i.totalSizeMB} MB).`)})}),d.commands.registerCommand("antigravityAnywhere.deleteConversation",async(e,i)=>{if(!e||e==="global-config"||!i&&await d.window.showWarningMessage(`Are you sure you want to delete conversation "${e}" and all its associated files?`,"Yes, Delete Conversation","Cancel")!=="Yes, Delete Conversation")return;let n=I(),c=ze(n.antigravityDataDir,e);try{let a=!1;for(let s of c)L.existsSync(s)&&(await L.promises.rm(s,{recursive:!0,force:!0}),a=!0);a&&(t.refresh(),M.refreshCurrentPanel(),d.window.showInformationMessage(`Antigravity Anywhere: Deleted conversation ${e}`))}catch(a){d.window.showErrorMessage(`Antigravity Anywhere: Failed to delete conversation: ${a.message}`)}}),d.commands.registerCommand("antigravityAnywhere.deleteBatchConversations",async e=>{if(!Array.isArray(e)||e.length===0)return;let i=I();for(let n of e){if(!n)continue;let c=ze(i.antigravityDataDir,n);for(let a of c)if(L.existsSync(a))try{await L.promises.rm(a,{recursive:!0,force:!0})}catch{}}t.refresh(),M.refreshCurrentPanel(),d.window.showInformationMessage(`Antigravity Anywhere: Successfully deleted ${e.length} selected conversation(s).`)}),d.commands.registerCommand("antigravityAnywhere.deleteFile",async(e,i)=>{if(!e||!i&&await d.window.showWarningMessage(`Are you sure you want to delete "${e}"?`,"Yes, Delete","Cancel")!=="Yes, Delete")return;let n=I(),c=C.dirname(n.antigravityDataDir),a=C.join(n.antigravityDataDir,e);if(e.startsWith("config/"))a=C.join(c,e);else if(e.startsWith("app_support/")){let s=P.getAppSupportDir();a=C.join(s,e.substring(12))}try{L.existsSync(a)&&(await L.promises.rm(a,{recursive:!0,force:!0}),t.refresh(),M.refreshCurrentPanel(),d.window.showInformationMessage(`Antigravity Anywhere: Deleted ${e}`))}catch(s){d.window.showErrorMessage(`Antigravity Anywhere: Failed to delete file: ${s.message}`)}}),d.commands.registerCommand("antigravityAnywhere.deleteAllFiles",async e=>{let i=I();if(!e){if(await d.window.showWarningMessage("\u26A0\uFE0F DANGER: Are you sure you want to DELETE ALL local conversation files in brain/, conversations/, and implicit/?","Yes, Delete All Files","Cancel")!=="Yes, Delete All Files")return;if(await d.window.showInputBox({prompt:"Type DELETE to confirm wiping all local conversation files:",placeHolder:"DELETE",ignoreFocusOut:!0})!=="DELETE"){d.window.showInformationMessage("Antigravity Anywhere: Delete All cancelled.");return}}try{await Z.createSnapshot(i.antigravityDataDir);let n=C.dirname(i.antigravityDataDir),c=P.getAppSupportDir(),a=[C.join(i.antigravityDataDir,"brain"),C.join(i.antigravityDataDir,"conversations"),C.join(i.antigravityDataDir,"implicit"),C.join(n,"antigravity","brain"),C.join(n,"antigravity","conversations"),C.join(n,"antigravity","implicit"),C.join(n,"antigravity-ide","brain"),C.join(n,"antigravity-ide","conversations"),C.join(n,"antigravity-ide","implicit"),C.join(c,"shared_proto_db")];for(let l of a)L.existsSync(l)&&await L.promises.rm(l,{recursive:!0,force:!0});let s=P.getCacheFilePath(i.antigravityDataDir);L.existsSync(s)&&await L.promises.unlink(s),t.refresh(),M.refreshCurrentPanel(),d.window.showInformationMessage("Antigravity Anywhere: All local conversation files wiped (safety snapshot created).")}catch(n){d.window.showErrorMessage(`Antigravity Anywhere: Delete All failed: ${n.message}`)}}));let o=I();o.enableAutoSync&&o.googleDriveToken&&(F=new V(o.syncIntervalSeconds,()=>re(!1)),F.startWatching(o.antigravityDataDir)),r.subscriptions.push(d.workspace.onDidChangeConfiguration(e=>{e.affectsConfiguration("antigravityAnywhere")&&Qe()}))}function Qe(){F&&(F.stopWatching(),F=null);let r=I();r.enableAutoSync&&r.googleDriveToken&&(F=new V(r.syncIntervalSeconds,()=>re(!1)),F.startWatching(r.antigravityDataDir))}function $(r,t,o,e="\u{1F680} Syncing to Google Drive..."){let i={command:"syncProgress",progress:{active:r,percentage:t,statusText:o,title:e}};M.currentPanel&&M.currentPanel.webview.postMessage(i),H.currentView&&H.currentView.webview.postMessage(i)}async function re(r=!1){let t=I();if(!t.googleDriveToken){r&&d.window.showWarningMessage("Antigravity Anywhere: Please sign in with Google first.","Sign In").then(e=>{e==="Sign In"&&d.commands.executeCommand("antigravityAnywhere.googleLogin")});return}G=new AbortController;let o=async()=>{try{B("Syncing...","$(sync~spin)"),$(!0,10,"\u{1F50D} Scanning local chat transcripts & databases... (Do not close Antigravity IDE)","\u{1F680} Syncing to Google Drive...");let e=await P.scanForSync(t.antigravityDataDir);if(G?.signal.aborted)throw new Error("Operation canceled by user.");if(e.files.length===0){r&&d.window.showInformationMessage("Antigravity Anywhere: No chat data found to sync."),B("Idle","$(cloud-check)"),$(!1,0,"");return}if(e.manifestHash===Be){B("Up to Date","$(cloud-check)"),$(!0,100,"\u26A1 Already Up to Date! No changes detected since last sync.","\u{1F680} Up to Date"),r&&d.window.showInformationMessage(`Antigravity Anywhere: Cloud backup is already up-to-date! All ${e.files.length} conversation files match Google Drive.`);return}$(!0,30,`\u26A1 Processing ${e.files.length} chat files (${e.totalSizeMB} MB)...`,"\u{1F680} Syncing to Google Drive...");let i=JSON.stringify(e);if(G?.signal.aborted)throw new Error("Operation canceled by user.");$(!0,55,"\u{1F512} Compressing & Encrypting payload (AES-256-GCM)...","\u{1F680} Syncing to Google Drive...");let n=De(i,t.encryptionPassword);if(G?.signal.aborted)throw new Error("Operation canceled by user.");$(!0,75,`\u2601\uFE0F Uploading encrypted bundle (${(n.length/(1024*1024)).toFixed(2)} MB) to Google Drive... Please wait...`,"\u{1F680} Syncing to Google Drive...");let c=new O(t.googleDriveToken,G?.signal),a=t.driveFileId;if(!a){let l=await c.findBackupFileId();l&&(a=l)}let s=await c.uploadSyncPayload(n,a,(l,g)=>{let x=Math.min(89,Math.floor(75+l/g*14)),w=(l/(1024*1024)).toFixed(1),v=(g/(1024*1024)).toFixed(1);$(!0,x,`\u2601\uFE0F Uploading: ${w} MB / ${v} MB (${x}%)... Please wait...`,"\u{1F680} Uploading to Google Drive...")});if(s!==t.driveFileId&&await le(s),$(!0,90,"\u{1F4BE} Saving local delta state cache...","\u{1F680} Syncing to Google Drive..."),await P.saveDeltaState(t.antigravityDataDir,e.files),Be=e.manifestHash,B("Synced","$(cloud-check)"),$(!0,100,`\u2705 Synced ${e.files.length} chat files (${e.totalSizeMB} MB) to Google Drive!`,"\u{1F680} Sync Complete"),r){let l=P.groupFilesByConversation(e),g=e.isIncremental?"\u26A1 Incremental Delta Sync":"\u{1F504} Full Backup Sync";d.window.showInformationMessage(`Antigravity Anywhere [${g}]: Synced ${l.length} active/modified chats (${e.files.length} files, ${e.totalSizeMB} MB) to Google Drive!`)}}catch(e){B("Sync Error","$(error)"),$(!0,0,`\u274C Sync Failed: ${e.message}`,"Sync Error"),r&&!G?.signal.aborted&&(e.message&&e.message.includes("console.developers.google.com")?d.window.showErrorMessage("Antigravity Cloud: Google Drive API is disabled in your Google Cloud Project (627024998523). Click below to enable it.","Enable Google Drive API").then(i=>{i==="Enable Google Drive API"&&d.env.openExternal(d.Uri.parse("https://console.developers.google.com/apis/api/drive.googleapis.com/overview?project=627024998523"))}):d.window.showErrorMessage(`Antigravity Anywhere Google Drive Sync Failed: ${e.message}`))}finally{G=null}};r?await d.window.withProgress({location:d.ProgressLocation.Notification,title:"Antigravity Anywhere: Syncing all conversations to Google Drive...",cancellable:!1},o):await o()}async function Ke(){let r=I();if(!r.googleDriveToken){d.window.showErrorMessage("Antigravity Anywhere: Google Sign-In is required to restore backups.","Sign In").then(i=>{i==="Sign In"&&d.commands.executeCommand("antigravityAnywhere.googleLogin")});return}G=new AbortController;let t=new O(r.googleDriveToken,G.signal),o=r.driveFileId;if(!o){B("Searching Google Drive...","$(sync~spin)"),$(!0,10,"\u{1F50D} Searching Google Drive for backup file...","\u{1F4E5} Restoring from Google Drive...");let i=await t.findBackupFileId();if(i)o=i,await le(o);else{B("Restore Error","$(error)"),$(!0,0,"\u274C No backup found on Google Drive","Restore Error"),d.window.showErrorMessage("Antigravity Anywhere: No cloud backup found in Google Drive account. Perform a Sync first on your main computer."),G=null;return}}if(await d.window.showWarningMessage("Restoring from Google Drive will update local chat history and SQLite databases. A local backup snapshot will be created automatically. Proceed?","Yes, Restore","Cancel")!=="Yes, Restore"){B("Idle","$(cloud-check)"),G=null;return}await d.window.withProgress({location:d.ProgressLocation.Notification,title:"Antigravity Anywhere: Restoring all conversations from Google Drive...",cancellable:!1},async()=>{try{B("Restoring...","$(sync~spin)"),$(!0,20,"\u{1F4F8} Creating Safety Snapshot local backup...","\u{1F4E5} Restoring from Google Drive..."),await Z.createSnapshot(r.antigravityDataDir),$(!0,45,"\u2601\uFE0F Downloading encrypted payload from Google Drive...","\u{1F4E5} Restoring from Google Drive...");let i=await t.downloadSyncPayload(o);$(!0,70,"\u{1F513} Decrypting payload & uncompressing files...","\u{1F4E5} Restoring from Google Drive...");let n=Ae(i,r.encryptionPassword),c=JSON.parse(n);$(!0,85,`\u{1F4C1} Restoring ${c.files?.length||0} transcript files and databases to disk...`,"\u{1F4E5} Restoring from Google Drive...");let a=await P.restoreBundle(r.antigravityDataDir,c);B("Restored","$(cloud-check)"),$(!0,100,`\u2705 Restored ${a} chat files! \u26A0\uFE0F Please Quit & Restart Antigravity IDE (Cmd+Q) to reload history.`,"\u{1F4E5} Restore Complete (Restart App)"),d.window.showInformationMessage(`Antigravity Cloud: Successfully restored ${a} conversation & database files! Please Quit & Restart Antigravity IDE to reload SQLite chat history.`,"\u{1F6AA} Quit Antigravity","\u{1F504} Reload Window","Later").then(s=>{s==="\u{1F6AA} Quit Antigravity"?d.commands.executeCommand("workbench.action.quit"):s==="\u{1F504} Reload Window"&&d.commands.executeCommand("workbench.action.reloadWindow")})}catch(i){B("Restore Error","$(error)"),$(!0,0,`\u274C Restore Failed: ${i.message}`,"Restore Error"),d.window.showErrorMessage(`Antigravity Anywhere Google Drive Restore Failed: ${i.message}`)}finally{G=null}})}function ze(r,t){let o=C.dirname(r),e=P.getAppSupportDir();if(t==="global-config")return[C.join(r,"config"),C.join(r,"state.vscdb"),C.join(r,"state.vscdb.backup"),C.join(o,"antigravity","config"),C.join(o,"antigravity-ide","config"),C.join(e,"shared_proto_db")];let i=[r,C.join(o,"antigravity"),C.join(o,"antigravity-ide")],n=[];for(let c of i)n.push(C.join(c,"brain",t),C.join(c,"conversations",`${t}.db`),C.join(c,"conversations",`${t}.db-wal`),C.join(c,"conversations",`${t}.db-shm`),C.join(c,"implicit",`${t}.pb`));return n}function B(r,t){K&&(K.text=`${t} Antigravity: ${r}`)}function Ze(){F&&F.stopWatching()}0&&(module.exports={activate,deactivate});
