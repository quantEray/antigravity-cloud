...
</html>`;t.webview.html=S}};var $=k(require("vscode"));var q=class r{constructor(t){this._extensionUri=t}static viewType="antigravityAnywhereSidebarView";static currentView;_view;resolveWebviewView(t,i,e){this._view=t,r.currentView=t,t.webview.options={enableScripts:!0,localResourceRoots:[this._extensionUri]},t.webview.onDidReceiveMessage(async o=>{switch(o.command){case"googleLogin":await $.commands.executeCommand("antigravityAnywhere.googleLogin"),this.refresh();break;case"googleLogout":await $.commands.executeCommand("antigravityAnywhere.googleLogout"),this.refresh();break;case"deepScan":await $.commands.executeCommand("antigravityAnywhere.deepScan"),this.refresh();break;case"syncNow":await $.commands.executeCommand("antigravityAnywhere.syncNow"),this.refresh();break;case"restore":await $.commands.executeCommand("antigravityAnywhere.restore"),this.refresh();break;case"deleteAll":await $.commands.executeCommand("antigravityAnywhere.deleteAllFiles",!0),this.refresh();break;case"deleteFile":await $.commands.executeCommand("antigravityAnywhere.deleteFile",o.relativePath),this.refresh();break;case"deleteConversation":await $.commands.executeCommand("antigravityAnywhere.deleteConversation",o.convId),this.refresh();break;case"openDashboard":await $.commands.executeCommand("antigravityAnywhere.openDashboard");break;case"cancelSync":await $.commands.executeCommand("antigravityAnywhere.cancelSync"),this.refresh();break;case"toggleAutoSync":await $.commands.executeCommand("antigravityAnywhere.toggleAutoSync"),this.refresh();break;case"openGithubStar":$.env.openExternal($.Uri.parse("https://github.com/quantEray/antigravity-cloud"));break;case"openGithubIssues":$.env.openExternal($.Uri.parse("https://github.com/quantEray/antigravity-cloud/issues"));break;case"setEncryptionPassword":await $.commands.executeCommand("antigravityAnywhere.setEncryptionPassword"),this.refresh();break;case"refresh":this.refresh();break}}),this.refresh()}async refresh(){this._view&&(this._view.webview.html=await this._getHtmlForWebview())}async _getHtmlForWebview(){let t=E(),i=Q.getDeviceInfo(),e=0,o="0",n="";try{let h=await T.scanDataDirectory(t.antigravityDataDir);o=h.totalSizeMB;let b=T.groupFilesByConversation(h,t.antigravityDataDir);e=b.length,n=b.map(v=>`<div class="conv-item">
              <div class="conv-header">
...
</html>`}};var L=k(require("vscode")),te=k(require("https")),Z=k(require("fs")),Le=k(require("path")),Ge=k(require("os")),oe=class r{strokeRepo="quantEray/antigravity-cloud";static async checkForUpdates(t,i=!1){let e=t.extension?.packageJSON?.version||"0.1.0";try{let o=await r.fetchLatestRelease();if(!o||!o.tag_name){i&&L.window.showInformationMessage("\u2601\uFE0F Antigravity Cloud: Unable to check for updates right now.");return}let n=o.tag_name.replace(/^v/,"");if(r.compareVersions(n,e)>0){let s=o.assets?.find(g=>g.name.endsWith(".vsix"))?.browser_download_url||`https://github.com/quantEray/antigravity-cloud/releases/download/${o.tag_name}/antigravity-anywhere-${n}.vsix`;await L.window.showInformationMessage(`\u{1F680} Antigravity Cloud update v${n} is available! (Current: v${e})`,"\u26A1 Update Now","Later")==="\u26A1 Update Now"&&await r.downloadAndInstallUpdate(s,n)}else i&&L.window.showInformationMessage(`\u2728 Antigravity Cloud is up to date (v${e}).`)}catch(o){i&&L.window.showErrorMessage(`Update check failed: ${o.message}`)}}static async fetchLatestRelease(){return new Promise(t=>{let e=te.request({hostname:"api.github.com",path:"/repos/quantEray/antigravity-cloud/releases/latest",method:"GET",headers:{"User-Agent":"Antigravity-Cloud-Extension"}},o=>{if(o.statusCode===301||o.statusCode===302){let c=o.headers.location;if(c){te.get(c,{headers:{"User-Agent":"Antigravity-Cloud-Extension"}},a=>{let s="";a.on("data",l=>s+=l),a.on("end",()=>{try{t(JSON.parse(s))}catch{t(null)}})});return}}let n="";o.on("data",c=>n+=c),o.on("end",()=>{try{t(JSON.parse(n))}catch{t(null)}})});e.on("error",()=>t(null)),e.end()})}static async downloadAndInstallUpdate(t,i){await L.window.withProgress({location:L.ProgressLocation.Notification,title:`\u{1F4E5} Downloading Antigravity Cloud v${i}...`,cancellable:!1},async e=>{let o=Le.join(Ge.tmpdir(),`antigravity-anywhere-${i}.vsix`);await r.downloadFile(t,o,e),e.report({message:"Installing extension package..."}),await L.commands.executeCommand("workbench.extensions.installExtension",L.Uri.file(o));try{Z.unlinkSync(o)}catch{}await L.window.showInformationMessage(`\u{1F389} Antigravity Cloud updated to v${i} successfully!`,"\u{1F504} Reload Window")==="\u{1F504} Reload Window"&&await L.commands.executeCommand("workbench.action.reloadWindow")})}static downloadFile(t,i,e){return new Promise((o,n)=>{let c=Z.createWriteStream(i),a=s=>{te.get(s,{headers:{"User-Agent":"Antigravity-Cloud-Extension"}},l=>{if((l.statusCode===301||l.statusCode===302)&&l.headers.location){a(l.headers.location);return}if(l.statusCode!==200){n(new Error(`Failed to download: HTTP status ${l.statusCode}`));return}let g=parseInt(l.headers["content-length"]||"0",10),h=0;l.on("data",b=>{if(h+=b.length,g>0){let v=Math.round(h/g*100);e.report({message:`${v}% (${(h/(1024*1024)).toFixed(2)} MB)`})}}),l.pipe(c),c.on("finish",()=>{c.close(),o()})}).on("error",l=>{Z.unlink(i,()=>n(l))})};a(t)})}static compareVersions(t,i){let e=t.split(".").map(Number),o=i.split(".").map(Number),n=Math.max(e.length,o.length);for(let c=0;c<n;c++){let a=e[c]||0,s=o[c]||0;if(a>s)return 1;if(a<s)return-1}return 0}};var X,z=null,Ue="",R=null;function ot(r){oe.checkForUpdates(r).catch(()=>{}),X=d.window.createStatusBarItem(d.StatusBarAlignment.Right,100),X.command="antigravityAnywhere.openDashboard",M("Not Logged In","$(account)"),X.show(),r.subscriptions.push(X);let t=new q(r.extensionUri);r.subscriptions.push(d.window.registerWebviewViewProvider(q.viewType,t)),W.validateOrRefreshToken().then(e=>{if(e){let o=E();M(o.googleUserName?`Signed in as ${o.googleUserName.split(" ")[0]}`:"Signed In","$(cloud-check)")}else M("Not Logged In","$(account)");t.refresh(),F.refreshCurrentPanel()}),r.subscriptions.push(d.commands.registerCommand("antigravityAnywhere.googleLogin",async()=>{try{M("Signing in...","$(sync~spin)"),await W.startLoginFlow();let e=E();M(e.googleUserName?`${e.googleUserName.split(" ")[0]}`:"Signed In","$(cloud-check)"),d.window.showInformationMessage("Antigravity Cloud: Successfully signed in with Google! You can now sync manually."),t.refresh(),F.refreshCurrentPanel()}catch(e){M("Login Error","$(error)"),d.window.showErrorMessage("Antigravity Cloud Google Login Failed: "+e.message)}}),d.commands.registerCommand("antigravityAnywhere.openDashboard",()=>F.show(r)),d.commands.registerCommand("antigravityAnywhere.checkForUpdates",async()=>{await oe.checkForUpdates(r,!0)}),d.commands.registerCommand("antigravityAnywhere.cancelSync",()=>{R&&(R.abort(),R=null,M("Canceled","$(x)"),B(!1,0,"\u23F9\uFE0F Sync canceled by user.","Canceled"),d.window.showInformationMessage("Antigravity Cloud: Sync operation canceled by user."),t.refresh(),F.refreshCurrentPanel())}),d.commands.registerCommand("antigravityAnywhere.toggleAutoSync",async()=>{let e=E(),o=!e.enableAutoSync;await ke(o),o?(e.googleDriveToken&&(z||(z=new J(e.syncIntervalSeconds,()=>ge(!1))),z.startWatching(e.antigravityDataDir)),d.window.showInformationMessage("Antigravity Cloud: Automatic Sync is now ENABLED \u26A1")):(z&&(z.stopWatching(),z=null),d.window.showInformationMessage("Antigravity Cloud: Automatic Sync is now DISABLED \u{1F6D1} (Manual Sync only)")),t.refresh(),F.refreshCurrentPanel()}),d.commands.registerCommand("antigravityAnywhere.syncNow",async()=>{await ge(!0),t.refresh(),F.refreshCurrentPanel()}),d.commands.registerCommand("antigravityAnywhere.restore",async()=>{await nt(),t.refresh(),F.refreshCurrentPanel()}),d.commands.registerCommand("antigravityAnywhere.setEncryptionPassword",async()=>{let e=E(),o=await d.window.showInputBox({prompt:"\u{1F511} Set Encryption Password for Antigravity Cloud backups (leave blank for default mode):",password:!0,value:e.encryptionPassword,placeHolder:"Enter custom encryption password..."});o!==void 0&&(await me(o.trim()),o.trim()?d.window.showInformationMessage("\u{1F511} Antigravity Cloud: Custom encryption password saved!"):d.window.showInformationMessage("\u{1F513} Antigravity Cloud: Encryption password cleared."),t.refresh(),F.refreshCurrentPanel())}),d.commands.registerCommand("antigravityAnywhere.googleLogout",async()=>{await W.logout(),M("Not Logged In","$(account)"),d.window.showInformationMessage("Antigravity Cloud: Signed out of Google."),t.refresh(),F.refreshCurrentPanel()}),d.commands.registerCommand("antigravityAnywhere.deepScan",async()=>{await d.window.withProgress({location:d.ProgressLocation.Notification,title:"Antigravity Anywhere: Deep Scanning all conversation directories...",cancellable:!1},async()=>{let e=E(),o=await T.scanDataDirectory(e.antigravityDataDir),n=T.groupFilesByConversation(o);t.refresh(),d.window.showInformationMessage(`Antigravity Anywhere Deep Scan Complete: Found ${n.length} Conversations (${o.files.length} Total Files, ${o.totalSizeMB} MB).`)})}),d.commands.registerCommand("antigravityAnywhere.deleteConversation",async(e,o)=>{if(!e||e==="global-config"||!o&&await d.window.showWarningMessage(`Are you sure you want to delete conversation "${e}" and all its associated files?`,"Yes, Delete Conversation","Cancel")!=="Yes, Delete Conversation")return;let n=E(),c=Ne(n.antigravityDataDir,e);try{let a=!1;for(let s of c)G.existsSync(s)&&(await G.promises.rm(s,{recursive:!0,force:!0}),a=!0);a&&(t.refresh(),F.refreshCurrentPanel(),d.window.showInformationMessage(`Antigravity Anywhere: Deleted conversation ${e}`))}catch(a){d.window.showErrorMessage(`Antigravity Anywhere: Failed to delete conversation: ${a.message}`)}}),d.commands.registerCommand("antigravityAnywhere.deleteBatchConversations",async e=>{if(!Array.isArray(e)||e.length===0)return;let o=E();for(let n of e){if(!n)continue;let c=Ne(o.antigravityDataDir,n);for(let a of c)if(G.existsSync(a))try{await G.promises.rm(a,{recursive:!0,force:!0})}catch{}}t.refresh(),F.refreshCurrentPanel(),d.window.showInformationMessage(`Antigravity Anywhere: Successfully deleted ${e.length} selected conversation(s).`)}),d.commands.registerCommand("antigravityAnywhere.deleteFile",async(e,o)=>{if(!e||!o&&await d.window.showWarningMessage(`Are you sure you want to delete "${e}"?`,"Yes, Delete","Cancel")!=="Yes, Delete")return;let n=E(),c=A.dirname(n.antigravityDataDir),a=A.join(n.antigravityDataDir,e);if(e.startsWith("config/"))a=A.join(c,e);else if(e.startsWith("app_support/")){let s=T.getAppSupportDir();a=A.join(s,e.substring(12))}try{G.existsSync(a)&&(await G.promises.rm(a,{recursive:!0,force:!0}),t.refresh(),F.refreshCurrentPanel(),d.window.showInformationMessage(`Antigravity Anywhere: Deleted ${e}`))}catch(s){d.window.showErrorMessage(`Antigravity Anywhere: Failed to delete file: ${s.message}`)}}),d.commands.registerCommand("antigravityAnywhere.deleteAllFiles",async e=>{let o=E();if(!e){if(await d.window.showWarningMessage("\u26A0\uFE0F DANGER: Are you sure you want to DELETE ALL local conversation files in brain/, conversations/, and implicit/?","Yes, Delete All Files","Cancel")!=="Yes, Delete All Files")return;if(await d.window.showInputBox({prompt:"Type DELETE to confirm wiping all local conversation files:",placeHolder:"DELETE",ignoreFocusOut:!0})!=="DELETE"){d.window.showInformationMessage("Antigravity Anywhere: Delete All cancelled.");return}}try{await ee.createSnapshot(o.antigravityDataDir);let n=A.dirname(o.antigravityDataDir),c=T.getAppSupportDir(),a=[A.join(o.antigravityDataDir,"brain"),A.join(o.antigravityDataDir,"conversations"),A.join(o.antigravityDataDir,"implicit"),A.join(n,"antigravity","brain"),A.join(n,"antigravity","conversations"),A.join(n,"antigravity","implicit"),A.join(n,"antigravity-ide","brain"),A.join(n,"antigravity-ide","conversations"),A.join(n,"antigravity-ide","implicit"),A.join(c,"shared_proto_db")];for(let l of a)G.existsSync(l)&&await G.promises.rm(l,{recursive:!0,force:!0});let s=T.getCacheFilePath(o.antigravityDataDir);G.existsSync(s)&&await G.promises.unlink(s),t.refresh(),F.refreshCurrentPanel(),d.window.showInformationMessage("Antigravity Anywhere: All local conversation files wiped (safety snapshot created).")}catch(n){d.window.showErrorMessage(`Antigravity Anywhere: Delete All failed: ${n.message}`)}}));let i=E();i.enableAutoSync&&i.googleDriveToken&&(z=new J(i.syncIntervalSeconds,()=>ge(!1)),z.startWatching(i.antigravityDataDir)),r.subscriptions.push(d.workspace.onDidChangeConfiguration(e=>{e.affectsConfiguration("antigravityAnywhere")&&it()}))}function it(){z&&(z.stopWatching(),z=null);let r=E();r.enableAutoSync&&r.googleDriveToken&&(z=new J(r.syncIntervalSeconds,()=>ge(!1)),z.startWatching(r.antigravityDataDir))}function B(r,t,i,e="\u{1F680} Syncing to Google Drive..."){let o={command:"syncProgress",progress:{active:r,percentage:t,statusText:i,title:e}};F.currentPanel&&F.currentPanel.webview.postMessage(o),q.currentView&&q.currentView.webview.postMessage(o)}async function ge(r=!1){let t=E();if(!t.googleDriveToken){r&&d.window.showWarningMessage("Antigravity Anywhere: Please sign in with Google first.","Sign In").then(e=>{e==="Sign In"&&d.commands.executeCommand("antigravityAnywhere.googleLogin")});return}R=new AbortController;let i=async()=>{try{M("Syncing...","$(sync~spin)"),B(!0,10,"\u{1F50D} Scanning local chat transcripts & databases... (Do not close Antigravity IDE)","\u{1F680} Syncing to Google Drive...");let e=await T.scanForSync(t.antigravityDataDir);if(R?.signal.aborted)throw new Error("Operation canceled by user.");if(e.files.length===0){r&&d.window.showInformationMessage("Antigravity Anywhere: No chat data found to sync."),M("Idle","$(cloud-check)"),B(!1,0,"");return}if(e.manifestHash===Ue){M("Up to Date","$(cloud-check)"),B(!0,100,"\u26A1 Already Up to Date! No changes detected since last sync.","\u{1F680} Up to Date"),r&&d.window.showInformationMessage(`Antigravity Anywhere: Cloud backup is already up-to-date! All ${e.files.length} conversation files match Google Drive.`);return}B(!0,30,`\u26A1 Processing ${e.files.length} chat files (${e.totalSizeMB} MB)...`,"\u{1F680} Syncing to Google Drive...");let o=JSON.stringify(e);if(R?.signal.aborted)throw new Error("Operation canceled by user.");B(!0,55,"\u{1F512} Compressing & Encrypting payload (AES-256-GCM)...","\u{1F680} Syncing to Google Drive...");let n=$e(o,t.encryptionPassword);if(R?.signal.aborted)throw new Error("Operation canceled by user.");B(!0,75,`\u2601\uFE0F Uploading encrypted bundle (${(n.length/(1024*1024)).toFixed(2)} MB) to Google Drive... Please wait...`,"\u{1F680} Syncing to Google Drive...");let c=new H(t.googleDriveToken,R?.signal),a=t.driveFileId;if(!a){let l=await c.findBackupFileId();l&&(a=l)}let s=await c.uploadSyncPayload(n,a,(l,g)=>{let h=Math.min(89,Math.floor(75+l/g*14)),b=(l/(1024*1024)).toFixed(1),v=(g/(1024*1024)).toFixed(1);B(!0,h,`\u2601\uFE0F Uploading: ${b} MB / ${v} MB (${h}%)... Please wait...`,"\u{1F680} Uploading to Google Drive...")});if(s!==t.driveFileId&&await ue(s),B(!0,90,"\u{1F4BE} Saving local delta state cache...","\u{1F680} Syncing to Google Drive..."),await T.saveDeltaState(t.antigravityDataDir,e.files),Ue=e.manifestHash,M("Synced","$(cloud-check)"),B(!0,100,`\u2705 Synced ${e.files.length} chat files (${e.totalSizeMB} MB) to Google Drive!`,"\u{1F680} Sync Complete"),r){let l=T.groupFilesByConversation(e),g=e.isIncremental?"\u26A1 Incremental Delta Sync":"\u{1F504} Full Backup Sync";d.window.showInformationMessage(`Antigravity Anywhere [${g}]: Synced ${l.length} active/modified chats (${e.files.length} files, ${e.totalSizeMB} MB) to Google Drive!`)}}catch(e){M("Sync Error","$(error)"),B(!0,0,`\u274C Sync Failed: ${e.message}`,"Sync Error"),r&&!R?.signal.aborted&&(e.message&&e.message.includes("console.developers.google.com")?d.window.showErrorMessage("Antigravity Cloud: Google Drive API is disabled in your Google Cloud Project (627024998523). Click below to enable it.","Enable Google Drive API").then(o=>{o==="Enable Google Drive API"&&d.env.openExternal(d.Uri.parse("https://console.developers.google.com/apis/api/drive.googleapis.com/overview?project=627024998523"))}):d.window.showErrorMessage(`Antigravity Anywhere Google Drive Sync Failed: ${e.message}`))}finally{R=null}};r?await d.window.withProgress({location:d.ProgressLocation.Notification,title:"Antigravity Anywhere: Syncing all conversations to Google Drive...",cancellable:!1},i):await i()}async function nt(){let r=E();if(!r.googleDriveToken){d.window.showErrorMessage("Antigravity Anywhere: Google Sign-In is required to restore backups.","Sign In").then(o=>{o==="Sign In"&&d.commands.executeCommand("antigravityAnywhere.googleLogin")});return}R=new AbortController;let t=new H(r.googleDriveToken,R.signal),i=r.driveFileId;if(!i){M("Searching Google Drive...","$(sync~spin)"),B(!0,10,"\u{1F50D} Searching Google Drive for backup file...","\u{1F4E5} Restoring from Google Drive...");let o=await t.findBackupFileId();if(o)i=o,await ue(i);else{M("Restore Error","$(error)"),B(!0,0,"\u274C No backup found on Google Drive","Restore Error"),d.window.showErrorMessage("Antigravity Anywhere: No cloud backup found in Google Drive account. Perform a Sync first on your main computer."),R=null;return}}if(await d.window.showWarningMessage("Restoring from Google Drive will update local chat history and SQLite databases. A local backup snapshot will be created automatically. Proceed?","Yes, Restore","Cancel")!=="Yes, Restore"){M("Idle","$(cloud-check)"),R=null;return}await d.window.withProgress({location:d.ProgressLocation.Notification,title:"Antigravity Anywhere: Restoring all conversations from Google Drive...",cancellable:!1},async()=>{try{M("Restoring...","$(sync~spin)"),B(!0,20,"\u{1F4F8} Creating Safety Snapshot local backup...","\u{1F4E5} Restoring from Google Drive..."),await ee.createSnapshot(r.antigravityDataDir),B(!0,45,"\u2601\uFE0F Downloading encrypted payload from Google Drive...","\u{1F4E5} Restoring from Google Drive...");let o=await t.downloadSyncPayload(i);B(!0,70,"\u{1F513} Decrypting payload & uncompressing files...","\u{1F4E5} Restoring from Google Drive...");let n=null,c=r.encryptionPassword;try{n=ve(o,c)}catch{n=null}if(!n){let l=c?"\u274C Saved encryption password is incorrect. Please enter the valid Encryption Password:":"\u{1F511} This cloud backup is encrypted with a password. Please enter the Encryption Password:";for(;!n;){let g=await d.window.showInputBox({prompt:l,password:!0,ignoreFocusOut:!0,placeHolder:"Enter your encryption password..."});if(g===void 0)throw new Error("Restore canceled: Valid encryption password is required to decrypt cloud backup.");try{n=ve(o,g.trim()),await me(g.trim()),d.window.showInformationMessage("\u{1F511} Antigravity Cloud: Valid encryption password saved!")}catch{l="\u274C Incorrect password. Please try again (or press Esc to cancel):"}}}let a=JSON.parse(n);B(!0,85,`\u{1F4C1} Restoring ${a.files?.length||0} transcript files and databases to disk...`,"\u{1F4E5} Restoring from Google Drive...");let s=await T.restoreBundle(r.antigravityDataDir,a);M("Restored","$(cloud-check)"),B(!0,100,`\u2705 Restored ${s} chat files! \u26A0\uFE0F Please Quit & Restart Antigravity IDE (Cmd+Q) to reload history.`,"\u{1F4E5} Restore Complete (Restart App)"),d.window.showInformationMessage(`Antigravity Cloud: Successfully restored ${s} conversation & database files! Please Quit & Restart Antigravity IDE to reload SQLite chat history.`,"\u{1F6AA} Quit Antigravity","\u{1F504} Reload Window","Later").then(l=>{l==="\u{1F6AA} Quit Antigravity"?d.commands.executeCommand("workbench.action.quit"):l==="\u{1F504} Reload Window"&&d.commands.executeCommand("workbench.action.reloadWindow")})}catch(o){M("Restore Error","$(error)"),B(!0,0,`\u274C Restore Failed: ${o.message}`,"Restore Error"),d.window.showErrorMessage(`Antigravity Anywhere Google Drive Restore Failed: ${o.message}`)}finally{R=null}})}function Ne(r,t){let i=A.dirname(r),e=T.getAppSupportDir();if(t==="global-config")return[A.join(r,"config"),A.join(r,"state.vscdb"),A.join(r,"state.vscdb.backup"),A.join(i,"antigravity","config"),A.join(i,"antigravity-ide","config"),A.join(e,"shared_proto_db")];let o=[r,A.join(i,"antigravity"),A.join(i,"antigravity-ide")],n=[];for(let c of o)n.push(A.join(c,"brain",t),A.join(c,"conversations",`${t}.db`),A.join(c,"conversations",`${t}.db-wal`),A.join(c,"conversations",`${t}.db-shm`),A.join(c,"implicit",`${t}.pb`));return n}function M(r,t){X&&(X.text=`${t} Antigravity: ${r}`)}function at(){z&&z.stopWatching()}0&&(module.exports={activate,deactivate});
