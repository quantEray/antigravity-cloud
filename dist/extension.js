"use strict";var ze=Object.create;var ee=Object.defineProperty;var Fe=Object.getOwnPropertyDescriptor;var Le=Object.getOwnPropertyNames;var Re=Object.getPrototypeOf,Ge=Object.prototype.hasOwnProperty;var Ue=(r,t)=>{for(var o in t)ee(r,o,{get:t[o],enumerable:!0})},fe=(r,t,o,e)=>{if(t&&typeof t=="object"||typeof t=="function")for(let i of Le(t))!Ge.call(r,i)&&i!==o&&ee(r,i,{get:()=>t[i],enumerable:!(e=Fe(t,i))||e.enumerable});return r};var A=(r,t,o)=>(o=r!=null?ze(Re(r)):{},fe(t||!r||!r.__esModule?ee(o,"default",{value:r,enumerable:!0}):o,r)),_e=r=>fe(ee({},"__esModule",{value:!0}),r);var Xe={};Ue(Xe,{activate:()=>Ye,deactivate:()=>Ze});module.exports=_e(Xe);var d=A(require("vscode")),R=A(require("fs")),C=A(require("path"));var I=A(require("vscode")),me=A(require("os")),ve=A(require("path"));function P(){let r=I.workspace.getConfiguration("antigravityAnywhere"),t=me.homedir(),o=ve.join(t,".gemini","antigravity-ide"),e=r.get("googleDriveToken","").trim(),i=r.get("googleRefreshToken","").trim(),a=r.get("driveFileId","").trim();return{googleDriveToken:e,googleRefreshToken:i,driveFileId:a,googleUserEmail:r.get("googleUserEmail","").trim(),googleUserName:r.get("googleUserName","").trim(),googleUserPicture:r.get("googleUserPicture","").trim(),encryptionPassword:r.get("encryptionPassword",""),enableAutoSync:r.get("enableAutoSync",!1),syncIntervalSeconds:r.get("syncIntervalSeconds",10),antigravityDataDir:o}}async function he(r){await I.workspace.getConfiguration("antigravityAnywhere").update("enableAutoSync",r,I.ConfigurationTarget.Global)}async function le(r){await I.workspace.getConfiguration("antigravityAnywhere").update("driveFileId",r,I.ConfigurationTarget.Global)}async function de(r,t){let o=I.workspace.getConfiguration("antigravityAnywhere");await o.update("googleDriveToken",r,I.ConfigurationTarget.Global),t&&await o.update("googleRefreshToken",t,I.ConfigurationTarget.Global)}async function be(r,t,o){let e=I.workspace.getConfiguration("antigravityAnywhere");await e.update("googleUserEmail",r,I.ConfigurationTarget.Global),await e.update("googleUserName",t,I.ConfigurationTarget.Global),await e.update("googleUserPicture",o,I.ConfigurationTarget.Global)}async function ye(){let r=I.workspace.getConfiguration("antigravityAnywhere");await r.update("googleDriveToken","",I.ConfigurationTarget.Global),await r.update("googleRefreshToken","",I.ConfigurationTarget.Global),await r.update("googleUserEmail","",I.ConfigurationTarget.Global),await r.update("googleUserName","",I.ConfigurationTarget.Global),await r.update("googleUserPicture","",I.ConfigurationTarget.Global),await r.update("driveFileId","",I.ConfigurationTarget.Global)}var k=A(require("fs")),y=A(require("path")),xe=A(require("os")),te=A(require("crypto"));var we=A(require("os")),W=class{static userHome=we.homedir();static normalize(t){if(!t)return t;let o=t.replace(/\\/g,"/"),e=this.userHome.replace(/\\/g,"/");return o.startsWith(e)&&(o=o.replace(e,"${USER_HOME}")),o}static denormalize(t){if(!t)return t;let o=this.userHome.replace(/\\/g,"/");return t.replace(/\${USER_HOME}/g,o)}};var T=class{static getAppSupportDir(){let t=xe.homedir();return process.platform==="win32"?process.env.APPDATA?y.join(process.env.APPDATA,"Antigravity IDE"):y.join(t,"AppData","Roaming","Antigravity IDE"):process.platform==="darwin"?y.join(t,"Library","Application Support","Antigravity IDE"):y.join(t,".config","Antigravity IDE")}static getCacheFilePath(t){return y.join(t,"delta_sync_state.json")}static loadDeltaState(t){try{let o=this.getCacheFilePath(t);if(k.existsSync(o)){let e=k.readFileSync(o,"utf-8");return JSON.parse(e)}}catch{}return null}static async saveDeltaState(t,o){try{let e=this.getCacheFilePath(t),i={};for(let c of o)i[c.relativePath]={mtimeMs:c.mtimeMs,sizeBytes:c.sizeBytes,hash:c.hash};let a={timestamp:new Date().toISOString(),filesState:i};await k.promises.mkdir(y.dirname(e),{recursive:!0}),await k.promises.writeFile(e,JSON.stringify(a,null,2),"utf-8")}catch{}}static async scanDataDirectory(t){let o=y.dirname(t),e=[],i=[t,y.join(o,"antigravity"),y.join(o,"antigravity-ide"),y.join(o,"antigravity-cli")],a=new Set;for(let s of i)if(k.existsSync(s))for(let n of["conversations","brain","implicit"]){let l=y.join(s,n);k.existsSync(l)&&!a.has(l)&&(a.add(l),await this.scanDirRecursive(l,s,e,!1))}let c=y.join(o,"config");return k.existsSync(c)&&!a.has(c)&&(a.add(c),await this.scanDirRecursive(c,o,e,!1)),this.buildBundle(e)}static async scanForSync(t,o=!1){let e=y.dirname(t),i=[],a=[t,y.join(e,"antigravity"),y.join(e,"antigravity-ide"),y.join(e,"antigravity-cli")],c=new Set;for(let v of a)if(k.existsSync(v))for(let h of["conversations","brain","implicit"]){let p=y.join(v,h);k.existsSync(p)&&!c.has(p)&&(c.add(p),await this.scanDirRecursive(p,v,i,!1))}let s=y.join(e,"config");k.existsSync(s)&&!c.has(s)&&(c.add(s),await this.scanDirRecursive(s,e,i,!1));let n=this.getAppSupportDir();if(k.existsSync(n)){let v=y.join(n,"shared_proto_db");k.existsSync(v)&&await this.scanDirRecursive(v,n,i,!1,"app_support");let h=y.join(n,"User","globalStorage","state.vscdb");if(k.existsSync(h))try{let p=await k.promises.stat(h);if(p.size<=10*1024*1024){let u="base64:"+(await k.promises.readFile(h)).toString("base64"),f="app_support/User/globalStorage/state.vscdb",m=te.createHash("sha256").update(u).digest("hex");i.push({relativePath:f,content:u,hash:m,sizeBytes:p.size,mtimeMs:p.mtimeMs})}}catch{}}let l=o?null:this.loadDeltaState(t),g=i,x=!1;if(l&&l.filesState){let v=new Set,h=new Set;for(let p of i){let S=l.filesState[p.relativePath],u=p.relativePath.startsWith("config/")||p.relativePath.startsWith("app_support/");if((!S||S.mtimeMs!==p.mtimeMs||S.sizeBytes!==p.sizeBytes)&&(h.add(p.relativePath),!u)){let f=p.relativePath.split("/"),m="";f[0]==="conversations"?m=f[1].replace(/\.(db|db-wal|db-shm)$/,""):f[0]==="brain"&&f.length>=2?m=f[1]:f[0]==="implicit"&&f.length>=2&&(m=f[1].replace(/\.pb$/,"")),m&&v.add(m)}}h.size>0&&h.size<i.length&&(x=!0,g=i.filter(p=>{if(p.relativePath.startsWith("config/")||p.relativePath.startsWith("app_support/"))return!0;let u=p.relativePath.split("/"),f="";return u[0]==="conversations"?f=u[1].replace(/\.(db|db-wal|db-shm)$/,""):u[0]==="brain"&&u.length>=2?f=u[1]:u[0]==="implicit"&&u.length>=2&&(f=u[1].replace(/\.pb$/,"")),v.has(f)||h.has(p.relativePath)}))}g.sort((v,h)=>h.mtimeMs-v.mtimeMs);let w=this.buildBundle(g);return w.isIncremental=x,w}static buildBundle(t){let o=[...t].sort((s,n)=>s.relativePath.localeCompare(n.relativePath)),e=o.reduce((s,n)=>s+n.sizeBytes,0),i=(e/(1024*1024)).toFixed(2),a=o.map(s=>`${s.relativePath}:${s.hash}`).join(`
`),c=te.createHash("sha256").update(a).digest("hex");return{timestamp:new Date().toISOString(),totalSizeBytes:e,totalSizeMB:i,files:o,manifestHash:c}}static groupFilesByConversation(t,o){let e=new Map,i=[],a=o?this.loadDeltaState(o):null;for(let s of t.files){let n=s.relativePath.split("/"),l="global-config";if(n[0]==="brain"&&n.length>1)l=n[1];else if(n[0]==="conversations"&&n.length>1)l=n[1].replace(/\.(db|db-wal|db-shm)$/,"");else if(n[0]==="implicit"&&n.length>1)l=n[1].replace(/\.pb$/,"");else{i.push(s);continue}e.has(l)||e.set(l,[]),e.get(l).push(s)}let c=[];for(let[s,n]of e.entries()){let l="",g=n.reduce((S,u)=>S+u.sizeBytes,0),x=(g/(1024*1024)).toFixed(2),w=Math.max(...n.map(S=>S.mtimeMs),0),v=w>0?new Date(w).toLocaleDateString():"Unknown";for(let S of n){if(S.relativePath.endsWith("metadata.json"))try{let u=JSON.parse(S.content);u.Summary?l=u.Summary.substring(0,65):u.title&&(l=u.title.substring(0,65))}catch{}else if(S.relativePath.endsWith("transcript.jsonl")){let u=S.content.split(`
`);for(let f of u)if(f.trim())try{let m=JSON.parse(f);if(m.type==="USER_INPUT"&&m.content){let D=m.content;if(D.includes("<USER_REQUEST>")){let b=D.match(/<USER_REQUEST>([\s\S]*?)<\/USER_REQUEST>/);b&&b[1]&&(D=b[1].trim())}if(D=D.replace(/^[\/\s\n\r\t]+/,"").trim(),D.length>10){l=D.substring(0,65);break}}}catch{}}if(l)break}if(!l){for(let S of n)if(S.relativePath.endsWith(".db"))try{let f=S.content.match(/[\x20-\x7E]{12,70}/g);if(f&&f.length>0){let m=f.filter(D=>D.length>=12&&/[a-zA-Z]/.test(D));for(let D of m){let b=D.replace(/[\r\n\t]+/g," ").replace(/\s+/g," ").trim();if(b=b.replace(/^[\/\s\n\r\t]+/,"").replace(/^SQLite format \d+/,"").trim(),b.length>=12&&!b.includes("sqlite")&&!b.includes("TABLE")&&!b.includes("INDEX")&&!b.includes("file:///")&&!b.includes("http")&&!b.includes("trajectory")&&!b.includes("battle_mode")&&!b.includes("Along with each USER request")&&!b.includes("conversation_summaries")&&!b.includes("System prompt")&&!b.includes("toolAction")&&!b.includes("PRIMARY KEY")&&!b.startsWith("function")&&!b.startsWith("import ")&&!b.startsWith("export ")&&!/^[0-9a-f\-]{30,}$/i.test(b)){l=b.substring(0,65);break}}}}catch{}}if(l?(l=l.replace(/^[\/\s\n\r\t]+/,"").replace(/^SQLite format \d+/,"").replace(/^\\n/,"").trim(),l.length>65&&(l=l.substring(0,65)+"...")):l=`Chat Session (${s.substring(0,8)})`,g<10240&&l.startsWith("Chat Session")&&s!=="global-config")continue;let p="local";a&&a.filesState&&(p=n.every(u=>{let f=a.filesState[u.relativePath];return f&&f.mtimeMs===u.mtimeMs&&f.sizeBytes===u.sizeBytes})?"synced":"modified"),c.push({id:s,title:l,totalSizeBytes:g,totalSizeMB:x,lastUpdated:v,status:p,files:n})}if(i.length>0){let s=i.reduce((l,g)=>l+g.sizeBytes,0),n="local";a&&a.filesState&&(n=i.every(g=>{let x=a.filesState[g.relativePath];return x&&x.mtimeMs===g.mtimeMs&&x.sizeBytes===g.sizeBytes})?"synced":"modified"),c.push({id:"global-config",title:"Global Config & System Indexes",totalSizeBytes:s,totalSizeMB:(s/(1024*1024)).toFixed(2),lastUpdated:"Current",status:n,files:i})}return c}static async scanDirRecursive(t,o,e,i=!1,a=""){let c=await k.promises.readdir(t,{withFileTypes:!0});for(let s of c){let n=y.join(t,s.name);if(s.isDirectory()){if(s.name==="node_modules"||s.name===".git"||s.name==="tasks")continue;await this.scanDirRecursive(n,o,e,i,a)}else if(s.isFile()){let l=y.extname(s.name).toLowerCase();if([".webp",".png",".jpg",".jpeg",".gif",".mp4",".webm",".zip",".gz"].includes(l)||s.name.endsWith(".log")&&!s.name.includes("transcript")&&!n.includes("shared_proto_db"))continue;try{let g=await k.promises.stat(n);if(g.size>50*1024*1024)continue;let x=[".db",".db-wal",".db-shm",".pb",".vscdb"].includes(l)||n.includes("shared_proto_db");if(i&&x)continue;let w="";if(x)w="base64:"+(await k.promises.readFile(n)).toString("base64");else{let p=await k.promises.readFile(n,"utf-8");w=W.normalize(p)}let v=y.relative(o,n).replace(/\\/g,"/");a&&(v=`${a}/${v}`);let h=te.createHash("sha256").update(w).digest("hex");e.push({relativePath:v,content:w,hash:h,sizeBytes:g.size,mtimeMs:g.mtimeMs})}catch{}}}}static async restoreBundle(t,o){let e=y.dirname(t),i=this.getAppSupportDir(),a=0,c=[t,y.join(e,"antigravity"),y.join(e,"antigravity-ide")];for(let s of o.files){let n=s.relativePath;if(s.relativePath.startsWith("config/")){let g=y.join(e,n.replace(/\//g,y.sep));try{await k.promises.mkdir(y.dirname(g),{recursive:!0}),s.content.startsWith("base64:")?await k.promises.writeFile(g,Buffer.from(s.content.substring(7),"base64")):await k.promises.writeFile(g,W.denormalize(s.content),"utf-8")}catch{}a++;continue}if(s.relativePath.startsWith("app_support/")){n=s.relativePath.substring(12);let g=y.join(i,n.replace(/\//g,y.sep));try{await k.promises.mkdir(y.dirname(g),{recursive:!0}),s.content.startsWith("base64:")?await k.promises.writeFile(g,Buffer.from(s.content.substring(7),"base64")):await k.promises.writeFile(g,W.denormalize(s.content),"utf-8")}catch{}a++;continue}let l=n.replace(/\//g,y.sep);for(let g of c){let x=y.join(g,l),w=y.dirname(x);try{if(await k.promises.mkdir(w,{recursive:!0}),s.content.startsWith("base64:")){let v=Buffer.from(s.content.substring(7),"base64");await k.promises.writeFile(x,v)}else{let v=W.denormalize(s.content);await k.promises.writeFile(x,v,"utf-8")}}catch{}}a++}return await this.saveDeltaState(t,o.files),a}};var U=A(require("crypto")),oe=A(require("zlib")),Se="aes-256-gcm",Ne=12,je=16,ke=32,Ce=1e5;function De(r,t){let e=oe.gzipSync(Buffer.from(r,"utf-8")).toString("base64");if(!t)return JSON.stringify({version:1,compressed:!0,unencrypted:!0,data:e});let i=U.randomBytes(je),a=U.randomBytes(Ne),c=U.pbkdf2Sync(t,i,Ce,ke,"sha256"),s=U.createCipheriv(Se,c,a),n=s.update(e,"utf-8","hex");n+=s.final("hex");let l=s.getAuthTag(),g={version:1,compressed:!0,salt:i.toString("hex"),iv:a.toString("hex"),tag:l.toString("hex"),data:n};return JSON.stringify(g)}function Ae(r,t){let o=JSON.parse(r),e="";if(o.unencrypted)e=o.data;else{if(!t)throw new Error("Encryption password is required to decrypt cloud backup.");let i=o,a=Buffer.from(i.salt,"hex"),c=Buffer.from(i.iv,"hex"),s=Buffer.from(i.tag,"hex"),n=i.data,l=U.pbkdf2Sync(t,a,Ce,ke,"sha256"),g=U.createDecipheriv(Se,l,c);g.setAuthTag(s);let x=g.update(n,"hex","utf-8");x+=g.final("utf-8"),e=x}return o.compressed?oe.gunzipSync(Buffer.from(e,"base64")).toString("utf-8"):Buffer.from(e,"base64").toString("utf-8")}var ae=A(require("https"));var pe=A(require("http")),ie=A(require("https")),ne=A(require("crypto")),q=A(require("vscode"));var ge="627024998523-13an3bmndm293rvgu9faomi6ao9bepks.apps.googleusercontent.com",Te="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",We=["https://www.googleapis.com/auth/drive.file","https://www.googleapis.com/auth/userinfo.profile","https://www.googleapis.com/auth/userinfo.email"].join(" "),Oe=12e4,N=class{static async startLoginFlow(){let t=await Ve(),o=`http://127.0.0.1:${t}`,e=Pe(ne.randomBytes(32)),i=Pe(ne.createHash("sha256").update(e).digest()),a=`https://accounts.google.com/o/oauth2/v2/auth?client_id=${encodeURIComponent(ge)}&redirect_uri=${encodeURIComponent(o)}&response_type=code&scope=${encodeURIComponent(We)}&access_type=offline&prompt=consent&code_challenge=${i}&code_challenge_method=S256`,c=qe(t);await q.env.openExternal(q.Uri.parse(a)),q.window.showInformationMessage("\u{1F511} Google sign-in page opened \u2014 authorize in browser to complete login.",{modal:!1});let s;try{s=await Je(c,Oe)}catch{throw new Error("Google Sign-In timed out or was cancelled. Please try again.")}let n=await He(s,e,o);await de(n.accessToken,n.refreshToken);try{await this.fetchAndStoreUserProfile(n.accessToken)}catch{}return n}static fetchAndStoreUserProfile(t){return new Promise((o,e)=>{let i={hostname:"www.googleapis.com",path:"/oauth2/v3/userinfo",method:"GET",headers:{Authorization:`Bearer ${t}`,"User-Agent":"Antigravity-Cloud-IDE"}},a=ie.request(i,c=>{let s="";c.on("data",n=>s+=n),c.on("end",async()=>{try{if(c.statusCode===200){let n=JSON.parse(s),l={email:n.email||"",name:n.name||n.email||"Google User",picture:n.picture||""};await be(l.email,l.name,l.picture),o(l)}else e(new Error(`Userinfo API error (${c.statusCode}): ${s}`))}catch(n){e(n)}})});a.on("error",c=>e(c)),a.end()})}static refreshAccessToken(t){return new Promise((o,e)=>{if(!t)return e(new Error("No Refresh Token available. Please sign in with Google again."));let i=new URLSearchParams({grant_type:"refresh_token",client_id:ge,client_secret:Te,refresh_token:t}).toString(),a={hostname:"oauth2.googleapis.com",path:"/token",method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded","Content-Length":Buffer.byteLength(i)}},c=ie.request(a,s=>{let n="";s.on("data",l=>n+=l),s.on("end",async()=>{try{let l=JSON.parse(n);l.access_token?(await de(l.access_token),this.fetchAndStoreUserProfile(l.access_token).catch(()=>{}),o(l.access_token)):e(new Error("Failed to refresh Google Access Token: "+(l.error_description||JSON.stringify(l))))}catch(l){e(new Error("Failed to parse refresh response: "+l.message))}})});c.on("error",s=>e(s)),c.write(i),c.end()})}static async logout(){await ye()}static async validateOrRefreshToken(){let t=P();if(!t.googleDriveToken)return!1;try{return await this.fetchAndStoreUserProfile(t.googleDriveToken),!0}catch{if(t.googleRefreshToken)try{let o=await this.refreshAccessToken(t.googleRefreshToken);return await this.fetchAndStoreUserProfile(o),!0}catch{return await this.logout(),!1}return await this.logout(),!1}}};function He(r,t,o){return new Promise((e,i)=>{let a=new URLSearchParams({code:r,client_id:ge,client_secret:Te,redirect_uri:o,grant_type:"authorization_code",code_verifier:t}).toString(),c={hostname:"oauth2.googleapis.com",path:"/token",method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded","Content-Length":Buffer.byteLength(a)}},s=ie.request(c,n=>{let l="";n.on("data",g=>l+=g),n.on("end",()=>{try{let g=JSON.parse(l);g.access_token?e({accessToken:g.access_token,refreshToken:g.refresh_token}):i(new Error("OAuth Token Exchange Error: "+(g.error_description||JSON.stringify(g))))}catch(g){i(new Error("Failed to parse token response: "+g.message))}})});s.on("error",n=>i(n)),s.write(a),s.end()})}function qe(r){return new Promise((t,o)=>{let e=pe.createServer((i,a)=>{try{let c=new URL(i.url||"/",`http://127.0.0.1:${r}`),s=c.searchParams.get("code"),n=c.searchParams.get("error");if(n){a.writeHead(200,{"Content-Type":"text/html; charset=utf-8"}),a.end(Ie("\u274C Sign-In Cancelled","You can close this tab and return to Antigravity IDE.",!1)),e.close(),o(new Error("Google Sign-In was denied: "+n));return}s&&(a.writeHead(200,{"Content-Type":"text/html; charset=utf-8"}),a.end(Ie("\u2705 Signed In!","You can close this tab and return to Antigravity IDE.",!0)),e.close(),t(s))}catch(c){o(c),e.close()}});e.on("error",o),e.listen(r,"127.0.0.1")})}function Ve(){return new Promise((r,t)=>{let o=pe.createServer();o.listen(0,"127.0.0.1",()=>{let i=o.address().port;o.close(()=>r(i))}),o.on("error",t)})}function Pe(r){return r.toString("base64").replace(/\+/g,"-").replace(/\//g,"_").replace(/=/g,"")}function Je(r,t){return new Promise((o,e)=>{let i=setTimeout(()=>e(new Error("Timeout")),t);r.then(a=>{clearTimeout(i),o(a)},a=>{clearTimeout(i),e(a)})})}function Ie(r,t,o){return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>
  ...
  <div class="action-bar">
    ...
    ${n?`<button class="btn btn-logout" onclick="sendMessage('googleLogout')">\u{1F6AA} Sign Out (${g.split(" ")[0]})</button>`:`<button class="btn btn-login" onclick="sendMessage('googleLogin')">\u{1F511} Sign in with Google</button>`}
      ...
      if (message.command === 'syncProgress') {
        ...
        if (card && active) {
          ...
          card.classList.remove('active');
            ...
            </div>`).join("")}catch{a='<div class="conv-item empty">No chat data found.</div>'}let c=!!t.googleDriveToken,s=t.googleUserEmail||"Google User",n=t.googleUserName||s,l=t.googleUserPicture||"",g=l?`<img src="${l}" class="user-avatar" alt="Avatar" referrerpolicy="no-referrer" onerror="this.onerror=null; this.src='https://lh3.googleusercontent.com/a/default-user';" />`:'<div class="user-avatar-fallback">\u{1F464}</div>';return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
...
<body>
  ...
  <div class="actions">
    ...
    ${c?`<button class="logout" onclick="send('googleLogout')">\u{1F6AA} Sign Out (${n.split(" ")[0]})</button>`:`<button class="login" onclick="send('googleLogin')">\u{1F511} Sign in with Google</button>`}
